...
public class ItemController {
    ...
    public ResponseEntity saveItem(@RequestBody ItemDTO item) {
        ...
        try (Connection connection = pool.getConnection()) {
            PreparedStatement stm = connection.prepareStatement("INSERT INTO item (description, stock, unitPrize) VALUES (?,?,?)", Statement.RETURN_GENERATED_KEYS);
            stm.setString(1,item.getDescription());
            stm.setString(2,item.getStock());
            stm.setString(3, item.getUnitPrize());
            stm.executeUpdate();
